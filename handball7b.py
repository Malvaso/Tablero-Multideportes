...
class MarcadorHandball(QWidget):
    ...
    def chequear_ganador(self):
		...
		# Eliminación matemática permitida antes de llegar a 5
        ...
        if tirosL5 == primeros and tirosV5 == primeros:
            if golesL5 != golesV5:
                self.ganador = "L" if golesL5 > golesV5 else "V"
                self.actualizar_penales_ui()
                #self.guardar_estado()
                return
			# sigue empate ? muerte súbita

		# --- Muerte súbita ---
		# Solo declarar cuando se completó la pareja: tiros iguales
