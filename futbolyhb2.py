...
class Marcador(QWidget):
    ...
    def cargar_estado(self):
        ...
        if getattr(self, "penales_activo", False):
            ...
            self.lbl_modo.setVisible(False)
        if self.modo_global:
            # ESTADO GUARDADO CON MODO GLOBAL ACTIVADO
            self.frame_global.setVisible(True) # Mostrar panel global
            
            # Ocultar marcador normal
    ...
    def initUI(self):
        ...
        layout_g.setVerticalSpacing(30)     # Separación vertical cómoda
        ...
        for i in range(3):
            ...
            lbl_tit.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            add_shadow(lbl_tit)
            ...
            lbl_name_L.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            ...
            lbl_sc_L.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            add_shadow(lbl_sc_L)
            ...
            lbl_dash.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            add_shadow(lbl_dash)
            ...
            lbl_sc_V.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            add_shadow(lbl_sc_V)
            ...
            lbl_name_V.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            add_shadow(lbl_name_V)
            layout_g.addWidget(lbl_name_V, i, 5)
            setattr(self, f"lbl_glob_name_V_{i}", lbl_name_V)

        # Ajuste de columnas para centrar bien los números
        layout_g.setColumnStretch(0, 0)
        layout_g.setColumnStretch(1, 2) # Nombres tienen más peso
        layout_g.setColumnStretch(2, 1)
        layout_g.setColumnStretch(3, 0)
        layout_g.setColumnStretch(4, 1)
        layout_g.setColumnStretch(5, 2) # Nombres tienen más peso
        ...
        for lbl in [self.lbl_local_txt, self.lbl_local_score, self.lbl_guion,
            ...
            # MANTENEMOS Ignored para evitar el loop infinito
            lbl.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored) 
            add_shadow(lbl)

        # --- AQUÍ ESTÁ EL ARREGLO DE VISIBILIDAD ---
        # Usamos pesos (stretch factors) en addWidget.
        # El número (e.g., 3, 2) le dice al layout: "Ocupa 3 partes del espacio",
        # forzando a que se vea aunque tenga política Ignored.
        
        left = QHBoxLayout()
        left.addStretch(1)                  # Spacer pequeño
        left.addWidget(self.lbl_local_txt, 3)   # Nombre grande
        left.addWidget(self.lbl_local_score, 2) # Número mediano
        left.addStretch(1)
        ...
        right = QHBoxLayout()
        right.addStretch(1)
        right.addWidget(self.lbl_visit_score, 2)
        right.addWidget(self.lbl_visit_txt, 3)
        right.addStretch(1)
        ...
        self.lbl_periodo.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
        ...
        # El tiempo NO debe ser Ignored verticalmente para no desaparecer,
        # pero horizontalmente sí para no ensanchar.
        self.lbl_tiempo.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Preferred)
        add_shadow(self.lbl_tiempo)
        
        # Estado
        self.lbl_estado = QLabel("Pausado")
        self.lbl_estado.setAlignment(Qt.AlignCenter)
        self.lbl_estado.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
        add_shadow(self.lbl_estado)
        
        # Modo
        self.lbl_modo = QLabel("Modo: ASCENDENTE")
        self.lbl_modo.setAlignment(Qt.AlignCenter)
        self.lbl_modo.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
        ...
        self.penales_lbl_title.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
    ...
    """
        ...
        self.frame_global.setStyleSheet("background: transparent;")
        layout_g = QGridLayout()
        layout_g.setVerticalSpacing(50)  # Antes estaba implícito o pequeño
        layout_g.setHorizontalSpacing(10)
        ...
        layout_g.setColumnStretch(0, 0) 
        layout_g.setColumnStretch(1, 2)
        layout_g.setColumnStretch(2, 1)
        layout_g.setColumnStretch(3, 0)
        layout_g.setColumnStretch(4, 1)
        layout_g.setColumnStretch(5, 2)
    """
    """
    ...
    def actualizar_tarjetas_ui(self):
        # 1. LIMPIEZA AGRESIVA: Ocultar y desvincular inmediatamente para evitar "fantasmas"
        viejas = self.tarjetas_local + self.tarjetas_visitante
        for t in viejas:
            t.hide()           # Desaparecer visualmente YA
            t.setParent(None)  # Desconectar de la ventana
            t.deleteLater()    # Programar borrado de memoria
        
        self.tarjetas_local = []
        self.tarjetas_visitante = []
        
        # Altura de la ventana actual
        screen_h = self.height()
        # Alto tarjeta: 4.5% de la pantalla (mínimo 15px para que no desaparezca)
        h = max(15, int(screen_h * 0.045))
        # Ancho tarjeta: proporcional (relación de aspecto ~0.7)
        w = int(h * 0.72)
        # Espacio entre tarjetas: 20% del ancho de la tarjeta
        espacio = max(2, int(w * 0.2))
        
        # --- DETERMINAR LOS LABELS "ANCLA" ---
        # Si estamos en modo global, queremos las tarjetas sobre los nombres de VUELTA (índice 1)
        if self.modo_global:
             # Usamos getattr por seguridad, por si se llama antes de initUI (raro pero posible)
             ...
             if target_L and target_L.isVisible():
                # En modo global, usamos mapTo para obtener la posición exacta dentro de la grilla
            ...
            # Función auxiliar para calcular la posición Y perfecta
            def calcular_pos_y(label):
                center_y = label.geometry().center().y()
                text_height = label.fontMetrics().height()
                # Posición: Centro - mitad texto - altura tarjeta - margen dinámico
                return center_y - (text_height // 2) - h - margen_y
            
            # --- DIBUJAR LOCAL ---
            if hasattr(self, "lbl_local_txt") and self.lbl_local_txt.isVisible(): #centrado horizontal en el bloque grande
                # no gemini x0 = self.lbl_local_txt.x()
                ...
                for i in range(self.rojas_local):
                    lbl = QLabel(self)
                    # no gemini lbl.setStyleSheet("background-color: red; border: 1px solid black;")
                    lbl.setStyleSheet(f"background-color: red; border: {max(1, int(w/20))}px solid black;")
                    lbl.setGeometry(x0 + i * (w + espacio), y0, w, h)
                    lbl.show()
                    #de gemini 
            ...
            if hasattr(self, "lbl_visit_txt") and self.lbl_visit_txt.isVisible():
                # no gemini x0 = self.lbl_visit_txt.x()
    ...
    def actualizar_tarjetas_ui(self):
        ...
        if target_L and target_L.isVisible():
            try:
                # mapTo nos da la posición absoluta en la ventana (Global o Normal)
                pos_L = target_L.mapTo(self, target_L.rect().topLeft())
                
                y0 = pos_L.y() - h - margen_y
                # Centrado matemático respecto al ancho del label
                total_w = (self.rojas_local * w) + ((self.rojas_local - 1) * espacio)
        ...
        if target_V and target_V.isVisible():
            try:
                pos_V = target_V.mapTo(self, target_V.rect().topLeft())
                y0 = pos_V.y() - h - margen_y
                total_w = (self.rojas_visitante * w) + ((self.rojas_visitante - 1) * espacio)
    ...
    """
    def actualizar_tarjetas_ui(self):
        # 1. Limpieza segura: Solo deleteLater y vaciar listas
        # No usamos setParent(None) ni hide() explícito para evitar SegFaults
        for t in self.tarjetas_local + self.tarjetas_visitante:
            try: 
                t.hide()
                t.deleteLater()
        ...
        # --- DIBUJAR LOCAL ---
        if target_L and target_L.isVisible():
            # Si estamos en modo global, usamos mapTo
            if self.modo_global:
                pos_L = target_L.mapTo(self, target_L.rect().topLeft())
                y0 = pos_L.y() - h - margen_y
                # Centrar en la celda de la grilla
                x0 = pos_L.x() + (target_L.width() - (self.rojas_local * w + (self.rojas_local - 1) * espacio)) // 2
            else:
                # Lógica clásica para modo normal (sin mapTo, usando coordenadas relativas al widget padre si es directo)
                # Como self.lbl_local_txt está en un layout, su .x() y .y() son relativos a la ventana/widget principal
                # pero a veces los layouts añaden márgenes. 
                # Usaremos geometry() directa que suele ser más estable en modo normal.
                rect = target_L.geometry()
                y0 = rect.center().y() - (target_L.fontMetrics().height() // 2) - h - margen_y
                x0 = rect.x() + (rect.width() - (self.rojas_local * w + (self.rojas_local - 1) * espacio)) // 2
        ...
        if target_V and target_V.isVisible():
            if self.modo_global:
                ...
                rect = target_V.geometry()
                y0 = rect.center().y() - (target_V.fontMetrics().height() // 2) - h - margen_y
                x0 = rect.x() + (rect.width() - (self.rojas_visitante * w + (self.rojas_visitante - 1) * espacio)) // 2
    ...
    def toggle_deporte(self):
        ...
        #Desactivar Modo Global obligatoriamente ---
        self.modo_global = False
        self.editando_global_fase = 0
        self.frame_global.setVisible(False)
    ...
    def render_menu(self):
        ...
        for i, (name, values) in enumerate(visible_items):
            ...
            if name == "Modo Tiempo":
                ...
                # Si estamos editando, lo pintamos de amarillo (HTML style)
    ...
    def actualizar_global_ui(self):
        ...
        # 1. Actualizar nombres (por si cambiaron)
        ...
        # Fila 1: Vuelta (Son los goles actuales del partido)
        ...
        # 4. Manejo de Colores (Edición)
        # Resetear todos a Negro
        estilo_normal = "color: black;"
        estilo_edit = "color: yellow; background-color: black; border-radius: 5px;"
        style_winner = "color: lime; font-weight: bold;"
        
        for lbl in self.lbls_global_scores_L + self.lbls_global_scores_V:
            lbl.setStyleSheet(estilo_normal)

        # Si estamos editando Ida Local (Fase 1)
        if self.editando_global_fase == 1:
            self.lbls_global_scores_L[0].setStyleSheet(estilo_edit)
        # Si estamos editando Ida Visitante (Fase 2)
        elif self.editando_global_fase == 2:
            self.lbls_global_scores_V[0].setStyleSheet(estilo_edit)
        
        # Nombres (Ganador Penales)
        # Primero resetear todos los nombres a negro
        for i in range(3):
            getattr(self, f"lbl_glob_name_L_{i}").setStyleSheet(estilo_normal)
            getattr(self, f"lbl_glob_name_V_{i}").setStyleSheet(estilo_normal)
        ...
        # 5. Ajuste de fuentes dinámico
        h = self.height()
        size_small = max(12, int(h * 0.035)) # Letra chica para Ida/Vuelta
        size_big = max(24, int(h * 0.065))   # Letra grande para Global

        font_s = QFont("Arial", size_small)
        font_b = QFont("Arial", size_big)

        # Aplicar fuentes
        # Filas 0 y 1 (Ida/Vuelta) pequeñas
        for i in [0, 1]:
