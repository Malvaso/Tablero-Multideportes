#ya no funcionan las tarjetas rojas en el modo normal, aunque se agreguen no se muestran
#!/usr/bin/env python3
import sys
import threading
import time
...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QSizePolicy, QFrame, QGraphicsDropShadowEffect, QGridLayout
...
# - S8: (sin uso)         (teclas[1][3])
# - S9: (sin uso)         (teclas[2][0])
# - S10: Back             (teclas[2][1])
# - S11: Up               (teclas[2][2])
...
class Marcador(QWidget):
    ...
    def __init__(self):
        ...
        self.tiempo = 0
        self.modo_reset = False
        self.opcion_reset_idx = 0  # 0=Todo, 1=Tiempo, 2=Penales
        self.modo_global = False
        self.goles_ida_local = 0
        self.goles_ida_visitante = 0
        # Estado de edición: 0=Nada, 1=Editando Ida Local, 2=Editando Ida Visitante
        self.editando_global_fase = 0
        # --- TARJETAS ROJAS ---
        self.rojas_local = 0
        self.rojas_visitante = 0
        self.opcion_roja_idx = 0      # 0 = local, 1 = visitante, 2 = borrar todas
        self.modo_tarjetas = False    # indica si estamos dentro del submenú de tarjetas
        self.tarjetas_local = []
        self.tarjetas_visitante = []
        self.tiempo_añadido = 0
        self.editando_añadido = False
        self.contando = False
        ...
        self.menu_futbol = [
            ("Añadir minutos extra", None),
            ("Cambiar periodo", None),
            ("Global ida y vuelta", ["OFF", "ON"]),
            ("Penales", ["OFF", "ON"]),
            ("Tarjeta roja", None),
            ("Establecer Tiempo Inicial", None),
            ("Cambiar nombre Local", None),
    ...
    def guardar_estado(self):
        estado = {
            ...
            "rojas_local": self.rojas_local,
            "rojas_visitante": self.rojas_visitante,
            "modo_global": self.modo_global,
            "goles_ida_local": self.goles_ida_local,
            "goles_ida_visitante": self.goles_ida_visitante,
            "tiempo_añadido": self.tiempo_añadido,
            "nombre_local": self.lbl_local_txt.text(),
            "nombre_visitante": self.lbl_visit_txt.text()
        }
        try:
    ...
    def cargar_estado(self):
        ...
        for t in getattr(self, "tarjetas_local", []) + getattr(self, "tarjetas_visitante", []):
            try: 
                t.hide()
                t.deleteLater()
            except: pass
        ...
        if estado:
            try:
                ...
                self.rojas_local = int(estado.get("rojas_local", 0))
                self.rojas_visitante = int(estado.get("rojas_visitante", 0))
                self.modo_global = bool(estado.get("modo_global", False))
                self.goles_ida_local = int(estado.get("goles_ida_local", 0))
                self.goles_ida_visitante = int(estado.get("goles_ida_visitante", 0))
                self.tiempo_añadido = int(estado.get("tiempo_añadido", 0))
                nom_local = estado.get("nombre_local", "LOCAL")
                nom_visit = estado.get("nombre_visitante", "VISITANTE")
                self.lbl_local_txt.setText(nom_local)
                self.lbl_visit_txt.setText(nom_visit)
                self.aplicar_fondo()

                self.penales_local = estado.get("penales_local", ["-"] * 5)
                self.penales_visitante = estado.get("penales_visitante", ["-"] * 5)
                
                maxlen = max(len(self.penales_local), len(self.penales_visitante))
                if len(self.penales_local) < maxlen:
                    self.penales_local += ["-"] * (maxlen - len(self.penales_local))
                if len(self.penales_visitante) < maxlen:
                    self.penales_visitante += ["-"] * (maxlen - len(self.penales_visitante))
            ...
            self.modo_global = False
            self.tiempo_añadido = 0
            # Nombres por defecto ya están puestos en el init
        ...
        for i in range(len(self.penales_local)):
            ...
            a.setAlignment(Qt.AlignCenter); a.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding); a.setFont(font_sym)
            self.penales_local_labels.append(a); self.row_local.addWidget(a)
            b = QLabel(self.penales_visitante[i])
            b.setAlignment(Qt.AlignCenter); b.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding); b.setFont(font_sym)
            self.penales_visit_labels.append(b); self.row_visit.addWidget(b)
        ...
        if self.deporte == "futbol":
            ...
            self.modo_global = False 
        """
        # 2. Pantalla principal (Prioridad: Penales > Global > Normal)
        if getattr(self, "penales_activo", False):
            self.penales_frame.setVisible(True)
            # Ocultar info central
            ...
            # Mostrar info central
            self.lbl_tiempo.setVisible(True)
            self.lbl_estado.setVisible(True)
            self.lbl_periodo.setVisible(True)

        # 3. Fondo (Global vs Normal)
        if self.modo_global:
            self.frame_global.setVisible(True)
            self.actualizar_global_ui()
            
            self.lbl_local_score.setVisible(False)
            self.lbl_visit_score.setVisible(False)
            self.lbl_guion.setVisible(False)
            self.lbl_local_txt.setVisible(False)
            self.lbl_visit_txt.setVisible(False)
        else:
            self.frame_global.setVisible(False)
            
            self.lbl_local_score.setVisible(True)
            self.lbl_visit_score.setVisible(True)
            self.lbl_guion.setVisible(True)
            self.lbl_local_txt.setVisible(True)
            self.lbl_visit_txt.setVisible(True)
        ...
        """
        # 5. DIBUJAR TODO (Tarjetas, Menú y Añadido)
        self.actualizar_ui_completa()
        self.actualizar_tarjetas_ui()
        self.render_menu()
    ...
    def procesar_boton(self, tecla):
        ...
        if self.teclado_activo:
            ...
            return
        #Lógica de edición Modo Global ---
        if self.editando_global_fase > 0:
            if tecla == "S11": # Arriba (+)
                if self.editando_global_fase == 1: self.goles_ida_local += 1
                elif self.editando_global_fase == 2: self.goles_ida_visitante += 1
                
            elif tecla == "S15": # Abajo (-)
                if self.editando_global_fase == 1: 
                    self.goles_ida_local = max(0, self.goles_ida_local - 1)
                elif self.editando_global_fase == 2: 
                    self.goles_ida_visitante = max(0, self.goles_ida_visitante - 1)
            
            elif tecla == "S12": # Enter (Siguiente paso)
                if self.editando_global_fase == 1:
                    self.editando_global_fase = 2 # Pasar a editar visitante
                else:
                    self.editando_global_fase = 0 # Terminar edición
        ...
        # Manejo de teclas cuando editamos minutos extra ---
        if getattr(self, "editando_añadido", False):
            if tecla in ("S11", "S15"):  # Arriba / Abajo
                self.editar_añadido_handler(tecla)
                return
            if tecla == "S12":  # Enter confirma
                self.editando_añadido = False
                self.render_menu()
                self.actualizar_añadido()
                return
            if tecla == "S10":  # Back sale
                self.editando_añadido = False
                self.render_menu()
                self.actualizar_añadido()
                return
            return # Bloquear otras teclas mientras editamos
            
        # --- SUBMENÚ EN UNA SOLA LÍNEA (TARJETAS ROJAS) ---
        if self.modo_tarjetas:
            if tecla in ("derecha", "S16"):
                self.opcion_roja_idx = (self.opcion_roja_idx + 1) % 3
                self.render_menu()
                return
        
            if tecla in ("izquierda", "S14"):
                self.opcion_roja_idx = (self.opcion_roja_idx - 1) % 3
                self.render_menu()
                return
        
            if tecla in ("enter", "S12"):
                self.aplicar_tarjeta_roja()
                self.modo_tarjetas = False
                self.render_menu()
                return
                
            # Back sale del modo tarjetas
            if tecla == "S10":
                self.modo_tarjetas = False
                self.render_menu()
                return
        
        #Manejo del submenú REINICIAR ---
        if self.modo_reset:
            if tecla in ("derecha", "S16"):
                self.opcion_reset_idx = (self.opcion_reset_idx + 1) % 3
                self.render_menu()
                return
            ...
            if tecla in ("enter", "S12"):
                self.aplicar_reset() # Función que crearemos abajo
                self.modo_reset = False
                self.render_menu()
                return
                
            # Back sale sin hacer nada
            if tecla == "S10":
                self.modo_reset = False
                self.render_menu()
                return
            
            return # Bloquear resto de teclas
    ...
    """
    def initUI(self):
        self.setWindowTitle("Tablero Multideportes")
        ...
        self.lbl_local_txt.installEventFilter(self)
        self.lbl_local_score = QLabel("0")
        self.lbl_guion = QLabel("-")
        self.lbl_visit_score = QLabel("0")
        self.lbl_visit_txt = QLabel("VISITANTE")
        self.lbl_visit_txt.installEventFilter(self)
        
        # Iconos de tarjetas rojas (en forma de labels)
        self.rojas_local_labels = []
        self.rojas_visit_labels = []
        # Contenedores horizontales para tarjetas rojas
        self.row_local_tarjetas = QHBoxLayout()
        self.row_visit_tarjetas = QHBoxLayout()
        # Insertarlos en el layout principal debajo del marcador
        self.layout_principal.addLayout(self.row_local_tarjetas)
        self.layout_principal.addLayout(self.row_visit_tarjetas)
        ...
        self.lbl_añadido.setStyleSheet(        color: white; font-size: 48px;)
        ...
        # --- Panel para Modo Global (Ida y Vuelta) ---
        self.frame_global = QFrame()
        self.frame_global.setStyleSheet("background: transparent;")
        layout_g = QGridLayout()
        layout_g.setVerticalSpacing(35)  # Antes estaba implícito o pequeño
        layout_g.setHorizontalSpacing(10)
        layout_g.setContentsMargins(5, 10, 5, 10) # Márgenes externos pequeños
        
        # Fuentes (se ajustarán en resize, pero definimos base aquí)
        self.font_small = QFont("Arial", 20)
        self.font_big = QFont("Arial", 40)

        # Filas: 0=Ida, 1=Vuelta, 2=Global
        labels_defs = ["IDA:", "VUELTA:", "GLOBAL:"]
        self.lbls_global_titulos = []
        self.lbls_global_scores_L = []
        self.lbls_global_scores_V = []
        self.lbls_global_dashes = []
        
        base_style_global = "color: black;"
        
        for i in range(3):
            # Título (IDA, VUELTA...)
            lbl_tit = QLabel(labels_defs[i]) 
            lbl_tit.setStyleSheet(base_style_global)
            lbl_tit.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
            add_shadow(lbl_tit)
            layout_g.addWidget(lbl_tit, i, 0)
            self.lbls_global_titulos.append(lbl_tit)
            
            # Nombre Local (copia visual)
            lbl_name_L = QLabel("LOCAL") 
            lbl_name_L.setStyleSheet(base_style_global)
            lbl_name_L.setAlignment(Qt.AlignCenter)
            add_shadow(lbl_name_L)
            layout_g.addWidget(lbl_name_L, i, 1)
            # Guardamos referencia para actualizar nombre
            setattr(self, f"lbl_glob_name_L_{i}", lbl_name_L)

            # Score Local
            lbl_sc_L = QLabel("0") 
            lbl_sc_L.setStyleSheet(base_style_global)
            lbl_sc_L.setAlignment(Qt.AlignCenter)
            add_shadow(lbl_sc_L)
            layout_g.addWidget(lbl_sc_L, i, 2)
            self.lbls_global_scores_L.append(lbl_sc_L)

            # Guion
            lbl_dash = QLabel("-") 
            lbl_dash.setStyleSheet(base_style_global)
            lbl_dash.setAlignment(Qt.AlignCenter)
            add_shadow(lbl_dash)
            layout_g.addWidget(lbl_dash, i, 3)
            self.lbls_global_dashes.append(lbl_dash)

            # Score Visitante
            lbl_sc_V = QLabel("0") 
            lbl_sc_V.setStyleSheet(base_style_global)
            lbl_sc_V.setAlignment(Qt.AlignCenter)
            add_shadow(lbl_sc_V)
            layout_g.addWidget(lbl_sc_V, i, 4)
            self.lbls_global_scores_V.append(lbl_sc_V)

            # Nombre Visitante
            lbl_name_V = QLabel("VISITANTE") 
            lbl_name_V.setStyleSheet(base_style_global)
            lbl_name_V.setAlignment(Qt.AlignCenter)
            add_shadow(lbl_name_V)
            layout_g.addWidget(lbl_name_V, i, 5)
            setattr(self, f"lbl_glob_name_V_{i}", lbl_name_V)

        # Ajuste de columnas para que se peguen al centro
        layout_g.setColumnStretch(0, 1) # Título empuja
        layout_g.setColumnStretch(5, 1) # Visitante empuja (si queremos centrar todo el bloque)
        
        self.frame_global.setLayout(layout_g)
        self.frame_global.setVisible(False) # Oculto al inicio
        
        # Insertamos este frame en el layout principal (antes del menú)
        self.layout_principal.insertWidget(0,self.frame_global)

        # ---------- MENÚ (aparece abajo, no tapa la información) ----------
        self.menu_frame = QFrame()
        self.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
        self.menu_frame.setFrameShape(QFrame.StyledPanel)
        self.menu_frame.setStyleSheet(      background-color: rgba(0, 0, 0, 160);        border-radius: 15px;)
    ...
    def initUI(self):
        ...
        # Función de sombra (igual que antes)
        def add_shadow(widget):
            sombra = QGraphicsDropShadowEffect()
            sombra.setBlurRadius(25)
            sombra.setXOffset(-3)
            sombra.setYOffset(3)
            sombra.setColor(QColor(255, 255, 255))
            widget.setGraphicsEffect(sombra)
        ...
        # Aplicar sombras y alineación a los normales
        for lbl in [self.lbl_local_txt, self.lbl_local_score, self.lbl_guion,
                    self.lbl_visit_score, self.lbl_visit_txt]:
            lbl.setAlignment(Qt.AlignCenter)
            lbl.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        # Layouts normales
        left = QHBoxLayout(); left.addStretch(); left.addWidget(self.lbl_local_txt); left.setSpacing(50); left.addWidget(self.lbl_local_score); left.addStretch()
        center = QHBoxLayout(); center.addWidget(self.lbl_guion)
        right = QHBoxLayout(); right.addStretch(); right.addWidget(self.lbl_visit_score); right.setSpacing(50); right.addWidget(self.lbl_visit_txt); right.addStretch()
        score_layout.addLayout(left, 1); score_layout.addLayout(center, 0); score_layout.addLayout(right, 1)
        self.layout_principal.addLayout(score_layout)

        # Periodo, Tiempo, Estado
        self.lbl_periodo = QLabel(self.periodos[self.periodo])
        self.lbl_periodo.setAlignment(Qt.AlignCenter)
        add_shadow(self.lbl_periodo)
        self.layout_principal.addWidget(self.lbl_periodo)

        self.lbl_tiempo = QLabel("00:00")
        self.lbl_tiempo.setAlignment(Qt.AlignCenter)
        add_shadow(self.lbl_tiempo)
        
        self.lbl_estado = QLabel("Pausado")
        self.lbl_estado.setAlignment(Qt.AlignCenter)
        add_shadow(self.lbl_estado)
        
        self.lbl_modo = QLabel("Modo: ASCENDENTE")
        self.lbl_modo.setAlignment(Qt.AlignCenter)
        add_shadow(self.lbl_modo)

        # Contenedor tiempo (Añadido)
        self.tiempo_container = QFrame()
        self.tiempo_container.setStyleSheet("background: transparent;")
        tc_layout = QVBoxLayout()
        tc_layout.setAlignment(Qt.AlignCenter)
        self.tiempo_container.setLayout(tc_layout)
        tc_layout.addWidget(self.lbl_tiempo)
        ...
        # --- PANEL GLOBAL (IDA / VUELTA / GLOBAL) ---
        self.frame_global = QFrame()
        self.frame_global.setStyleSheet("background: transparent;")
        
        layout_g = QGridLayout()
        layout_g.setVerticalSpacing(50) # Separación vertical cómoda
        layout_g.setHorizontalSpacing(10)
        layout_g.setContentsMargins(5, 10, 5, 10)
        ...
        base_style_global = "color: black; font-weight: bold;"
        ...
        # --- CORRECCIÓN DE ESPACIADO EN PANTALLAS ANCHAS ---
        # Col 0 (Título): Stretch 0 (Solo ocupa lo que necesita)
        # Col 1 (Local): Stretch 1 (Se estira para empujar números al centro)
        # Col 5 (Visitante): Stretch 1 (Se estira igual que el local)
        # El resto (numeros): Stretch 0
        layout_g.setColumnStretch(0, 0) 
        layout_g.setColumnStretch(1, 1)
        layout_g.setColumnStretch(2, 0)
        layout_g.setColumnStretch(3, 0)
        layout_g.setColumnStretch(4, 0)
        layout_g.setColumnStretch(5, 1)
        # ---------------------------------------------------
        ...
        self.penales_lbl_title = QLabel("PENALES")
        self.penales_lbl_title.setAlignment(Qt.AlignCenter)
        add_shadow(self.penales_lbl_title)
        p_layout.addWidget(self.penales_lbl_title)
        
        self.penales_local_labels = []
        self.penales_visit_labels = []
        self.row_local = QHBoxLayout(); self.row_visit = QHBoxLayout()
        self.row_local.setSpacing(10); self.row_visit.setSpacing(10)
        
        self.lbl_penales_L = QLabel("L:"); self.lbl_penales_L.setAlignment(Qt.AlignCenter); add_shadow(self.lbl_penales_L)
        self.lbl_penales_V = QLabel("V:"); self.lbl_penales_V.setAlignment(Qt.AlignCenter); add_shadow(self.lbl_penales_V)
        self.row_local.addWidget(self.lbl_penales_L)
        self.row_visit.addWidget(self.lbl_penales_V)

        for i in range(5):
            a = QLabel("-"); a.setAlignment(Qt.AlignCenter); a.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            b = QLabel("-"); b.setAlignment(Qt.AlignCenter); b.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            self.penales_local_labels.append(a); self.penales_visit_labels.append(b)
            self.row_local.addWidget(a); self.row_visit.addWidget(b)
        
        p_layout.addLayout(self.row_local); p_layout.addLayout(self.row_visit)
        self.penales_frame.setLayout(p_layout)
        self.penales_frame.setVisible(False)
        self.layout_principal.addWidget(self.penales_frame)
        ...
        self.menu_frame.setFrameShape(QFrame.StyledPanel)
        self.menu_frame.setStyleSheet("background-color: rgba(0, 0, 0, 160); border-radius: 15px;")
        menu_l = QVBoxLayout(); menu_l.setContentsMargins(8, 8, 8, 8); menu_l.setSpacing(6)
        
        self.visible_menu_rows = 3
        self.menu_labels = []
        for i in range(self.visible_menu_rows):
            lbl = QLabel()
            lbl.setAlignment(Qt.AlignLeft)
            ...
            lbl.setMinimumHeight(40)
            self.menu_labels.append(lbl)
            menu_l.addWidget(lbl)
            
        self.menu_frame.setLayout(menu_l)
        self.menu_frame.setVisible(False)
        self.layout_principal.addWidget(self.menu_frame)
        self.setLayout(self.layout_principal)
    ...
    def actualizar_tarjetas_ui(self):
        # 1. Limpieza segura
        for t in self.tarjetas_local + self.tarjetas_visitante:
            try: 
                t.hide()
                t.deleteLater()
            except: pass
        ...
        h = max(15, int(screen_h * 0.045))  # Alto tarjeta
        w = int(h * 0.72)                   # Ancho tarjeta
        espacio = max(2, int(w * 0.2))      # Separación

        # ---------------------------------------------------------
        # CASO 1: MODO GLOBAL (Usamos mapTo sobre la grilla)
        # ---------------------------------------------------------
        if self.modo_global:
            target_L = getattr(self, "lbl_glob_name_L_1", None)
            target_V = getattr(self, "lbl_glob_name_V_1", None)
            margen_y = 2
            
            if target_L and target_L.isVisible():
                pos_L = target_L.mapTo(self, target_L.rect().topLeft())
                y0 = pos_L.y() - h - margen_y
                # Centrado en celda
                total_w = (self.rojas_local * w) + (max(0, self.rojas_local - 1) * espacio)
                x0 = pos_L.x() + (target_L.width() - total_w) // 2

                for i in range(self.rojas_local):
                    lbl = QLabel(self)
                    lbl.setStyleSheet(f"background-color: red; border: {max(1, int(w/20))}px solid black;")
                    lbl.setGeometry(x0 + i * (w + espacio), y0, w, h)
                    lbl.show()
                    lbl.raise_()
                    self.tarjetas_local.append(lbl)

            if target_V and target_V.isVisible():
                pos_V = target_V.mapTo(self, target_V.rect().topLeft())
                y0 = pos_V.y() - h - margen_y
                total_w = (self.rojas_visitante * w) + (max(0, self.rojas_visitante - 1) * espacio)
                x0 = pos_V.x() + (target_V.width() - total_w) // 2

                for i in range(self.rojas_visitante):
                    lbl = QLabel(self)
                    lbl.setStyleSheet(f"background-color: red; border: {max(1, int(w/20))}px solid black;")
                    lbl.setGeometry(x0 + i * (w + espacio), y0, w, h)
                    lbl.show()
                    lbl.raise_()
                    self.tarjetas_visitante.append(lbl)

        # ---------------------------------------------------------
        # CASO 2: MODO NORMAL (Tu lógica clásica restaurada)
        # ---------------------------------------------------------
        else:
            margen_y = max(2, int(h * 0.15))
            
            def calcular_y_normal(label):
                center_y = label.geometry().center().y()
                return center_y - (label.fontMetrics().height() // 2) - h - margen_y

            # LOCAL
            if hasattr(self, "lbl_local_txt") and self.lbl_local_txt.isVisible():
                total_w = (self.rojas_local * w) + (max(0, self.rojas_local - 1) * espacio)
                # Cálculo matemático simple
                x0 = self.lbl_local_txt.x() + (self.lbl_local_txt.width() - total_w) // 2
                y0 = calcular_y_normal(self.lbl_local_txt)
            ...
            # VISITANTE
            if hasattr(self, "lbl_visit_txt") and self.lbl_visit_txt.isVisible():
                total_w = (self.rojas_visitante * w) + (max(0, self.rojas_visitante - 1) * espacio)
                # Cálculo matemático simple
                x0 = self.lbl_visit_txt.x() + (self.lbl_visit_txt.width() - total_w) // 2
                y0 = calcular_y_normal(self.lbl_visit_txt)
    ...
    def actualizar_tarjetas_ui(self):
        ...
        screen_h = self.height()
        h = max(15, int(screen_h * 0.045))
        w = int(h * 0.72)
        espacio = max(2, int(w * 0.2))
        
        # --- SELECCIÓN DE OBJETIVOS ---
        # Si el modo global está activo, las tarjetas van ahí SIEMPRE.
        # (La visibilidad ya está garantizada por actualizar_ui_completa)
        if self.modo_global:
            target_L = getattr(self, "lbl_glob_name_L_1", None)
            target_V = getattr(self, "lbl_glob_name_V_1", None)
            margen_y = 2
        else:
            target_L = self.lbl_local_txt
            target_V = self.lbl_visit_txt
            margen_y = max(2, int(h * 0.15))
    ...
    def actualizar_tarjetas_ui(self):
        ...
        h = max(15, int(screen_h * 0.045))  # Alto tarjeta
        w = int(h * 0.72)                   # Ancho tarjeta
        espacio = max(2, int(w * 0.2))      # Separación entre ellas

        # --- CASO 1: MODO GLOBAL ---
        if self.modo_global:
            # Usamos los labels de la fila 1 (VUELTA) como referencia
            target_L = getattr(self, "lbl_glob_name_L_1", None)
            target_V = getattr(self, "lbl_glob_name_V_1", None)            
            # Margen vertical pequeño para el global
            margen_y = 2 

            if target_L and target_L.isVisible():
                # En modo global, usamos mapTo para obtener la posición absoluta exacta dentro de la grilla
                pos_L = target_L.mapTo(self, target_L.rect().topLeft())
                y0 = pos_L.y() - h - margen_y
                # Centrar respecto al ancho del texto "LOCAL" en la celda
                x0 = pos_L.x() + (target_L.width() - (self.rojas_local * w + (self.rojas_local - 1) * espacio)) // 2
            ...
            if target_V and target_V.isVisible():
                pos_V = target_V.mapTo(self, target_V.rect().topLeft())
                y0 = pos_V.y() - h - margen_y
                x0 = pos_V.x() + (target_V.width() - (self.rojas_visitante * w + (self.rojas_visitante - 1) * espacio)) // 2
        ...
        # --- CASO 2: MODO NORMAL (Tu código original restaurado) ---
        else:
            # Margen dinámico original
            margen_y = max(2, int(h * 0.15))
            
            # Función auxiliar original
            def calcular_pos_y(label):
                center_y = label.geometry().center().y()
                text_height = label.fontMetrics().height()
                return center_y - (text_height // 2) - h - margen_y

            # LOCAL
            if hasattr(self, "lbl_local_txt") and self.lbl_local_txt.isVisible():
                # Centrado horizontal en el bloque grande
                x0 = self.lbl_local_txt.x() + (self.lbl_local_txt.width() - (self.rojas_local * w + (self.rojas_local - 1) * espacio)) // 2
                y0 = calcular_pos_y(self.lbl_local_txt)
            ...
            # VISITANTE
            if hasattr(self, "lbl_visit_txt") and self.lbl_visit_txt.isVisible():
                x0 = self.lbl_visit_txt.x() + (self.lbl_visit_txt.width() - (self.rojas_visitante * w + (self.rojas_visitante - 1) * espacio)) // 2
                y0 = calcular_pos_y(self.lbl_visit_txt)
    ...
    def aplicar_tarjeta_roja(self):
        if self.opcion_roja_idx == 0:      # LOCAL
            self.rojas_local += 1    
        elif self.opcion_roja_idx == 1:    # VISITANTE
            self.rojas_visitante += 1
        elif self.opcion_roja_idx == 2:    # BORRAR TODAS
            self.rojas_local = 0
            self.rojas_visitante = 0
    ...
    def actualizar_añadido(self):
        ...
        # Mostrar si: hay añadido, O pasó el tiempo, O estamos editándolo ahora mismo
        editando = getattr(self, "editando_añadido", False)
        mostrar = (self.tiempo_añadido > 0) or (tiempo_actual >= tiempo_limite) or editando
        
        if mostrar:
            self.lbl_añadido.setText(f" +{self.tiempo_añadido}")
            if editando:
                self.lbl_añadido.setStyleSheet("color: yellow; font-size: 48px;")
            else:
                self.lbl_añadido.setStyleSheet("color: white; font-size: 48px;")
    ...
    def editar_añadido_handler(self, tecla):
        # S11 = Arriba (+), S15 = Abajo (-)
        if tecla == "S11":
            self.tiempo_añadido += 1
        elif tecla == "S15":
            self.tiempo_añadido = max(0, self.tiempo_añadido - 1)
    ...
    def finalizar_edicion_nombre(self, nombre):
        ...
        if self.modo_global:
            self.actualizar_global_ui()
            # También actualizamos las tarjetas por si el nombre cambió de largo y se movió el centro
            self.actualizar_tarjetas_ui()
    ...
    def actualizar_espaciado_global(self):
        # Solo actuar si el modo global está encendido
        if not self.modo_global or self.frame_global.layout() is None:
            return

        # Si el menú está abierto, achicamos el espacio (10px).
        # Si está cerrado, usamos el espacio grande que te gusta (50px).
        nuevo_espacio = 10 if self.menu_activo else 50
        
        self.frame_global.layout().setVerticalSpacing(nuevo_espacio)
    ...
    def toggle_menu(self):
        ...
        self.actualizar_espaciado_global()
        self.render_menu()
    ...
    def menu_up(self):
        if self.modo_tarjetas or self.modo_reset:
            return
    ...
    def menu_enter(self):
        ...
        if name == "Cambiar periodo":
            self.cambiar_periodo()
            return
        # Entrar al modo reset ---
        if name == "Reiniciar":
            self.modo_reset = True
            self.opcion_reset_idx = 0 # Empezar en "Todo" por seguridad o defecto
            self.render_menu()
            return
        
        # --- OPCIONES EXCLUSIVAS DE FÚTBOL ---
        if self.deporte == "futbol":
            if name == "Global ida y vuelta":
                # Toggle (activar/desactivar)
                self.modo_global = not self.modo_global
                
                if self.modo_global:
                    # Activar UI Global, ocultar normal
                    self.frame_global.setVisible(True)
                    # Iniciar edición inmediata: Fase 1 (Ida Local)
                    self.editando_global_fase = 1
                    # Cerrar menú para ver la edición
                    self.menu_activo = False
                    self.menu_frame.setVisible(False)
                    self.actualizar_espaciado_global()
                else:
                ...
                # Actualizar visibilidad completa
                self.actualizar_ui_completa()
                self.actualizar_global_ui()
                return
                
            if name == "Añadir minutos extra":
                self.editando_añadido = True
                self.render_menu()
                # Forzamos que se muestre el cartel para ver lo que editamos
                self.actualizar_añadido() 
                return
            ...
            if name == "Tarjeta roja":
                self.modo_tarjetas = True
                self.opcion_roja_idx = 0
                self.render_menu()
                return
    ...
    def toggle_deporte(self):
        # 1. Resetear contadores de tarjetas
        self.rojas_local = 0
        self.rojas_visitante = 0
        self.actualizar_tarjetas_ui()

        # 2. --- NUEVO: Desactivar Modo Global obligatoriamente ---
        self.modo_global = False
        self.editando_global_fase = 0
        self.frame_global.setVisible(False)
        # ---------------------------------------------------------
        ...
        # 4. --- NUEVO: Restaurar la interfaz normal (scoreboards visibles) ---
        self.actualizar_ui_completa() 
    ...
    def render_menu(self):
        ...
        for i, (name, values) in enumerate(visible_items):
            index_real = start + i
            if self.modo_reset and name == "Reiniciar":
                # Convertir índice real a índice visible
                if start <= index_real < end:
                    visible_idx = index_real - start
                    opciones = ["Todo", "Tiempo", "Penales"]
                    html = "Reiniciar:  "
                    for k, t in enumerate(opciones):
                        if k == self.opcion_reset_idx:
                            html += f'<span style="background-color: yellow; color: black; padding: 4px; border-radius: 6px;">{t}</span>   '
                        else:
                            html += f'<span style="background-color: gray; color: white; padding: 4px; border-radius: 6px;">{t}</span>   '
                    self.menu_labels[visible_idx].setText(html)
                    # Forzamos continuar para no sobrescribir con la lógica estándar de abajo
                    continue
            ...
            # --- NUEVO: Calcular valor para Global ---
            elif name == "Global ida y vuelta":
                val = "ON" if self.modo_global else "OFF"
            # -----------------------------------------

            elif name == "Añadir minutos extra":
                num = f"+{self.tiempo_añadido}"
                if self.editando_añadido:
                    val = f"<span style='color:yellow;'>{num}</span>"
                else:
                    val = num

            elif name == "Establecer Tiempo Inicial":
                # ... (tu código existente de tiempo) ...
                ...
                if self.editando_tiempo:
                    ...
                    idx = [0,1,3,4][pos]
                    digits = (digits[:idx] + f"<span style='color:yellow;'>{digits[idx]}</span>" + digits[idx+1:])
                val = digits
            ...
            # Como ahora "Global..." tiene values != None, entrará automáticamente en el else
            if values is None and name != "Establecer Tiempo Inicial" and name != "Añadir minutos extra":
                txt = name
            else:
                txt = f"{name}: {val}"

            # ... (resto de asignación de texto al label y colores igual) ...
        # Construcción de la línea de texto
# ojo la INDENTACION ACA
            if values is None and name != "Establecer Tiempo Inicial" and name != "Añadir minuto extra":
                txt = name
            else:
                txt = f"{name}: {val}"
        ...
        # Mostrar las opciones de tarjeta roja EN EL MISMO RENGLÓN
        if self.modo_tarjetas:
            # encontrar el índice del item "Tarjeta roja"
            for idx, (name, values) in enumerate(self.menu_items):
                if name == "Tarjeta roja":
                    item_index = idx
                    break
            # convertir índice real a índice visible
            start = self.menu_top_index
            end = self.menu_top_index + self.visible_menu_rows
        
            if start <= item_index < end:
                # posición dentro de los labels visibles (0,1,2)
                visible_idx = item_index - start
                opciones = ["Local", "Visitante", "Borrar todas"]
                html = "Tarjeta roja:  "
                for i, t in enumerate(opciones):
                    if i == self.opcion_roja_idx:
                        html += f'<span style="background-color: yellow; color: black; padding: 4px; border-radius: 6px;">{t}</span>   '
                    else:
                        html += f'<span style="background-color: gray; color: white; padding: 4px; border-radius: 6px;">{t}</span>   '
                self.menu_labels[visible_idx].setText(html)
    ...
    def actualizar_penales_ui(self):
        # actualiza textos de labels de penales
        ...
        # aplicar color ganador en verde al marcador normal
        if self.ganador == "L":
            ...
            self.lbl_local_txt.setStyleSheet("color: black;")
            self.lbl_visit_txt.setStyleSheet("color: black;")
        #Forzar actualización del Global para ver el cambio de color ---
        if self.modo_global:
            self.actualizar_global_ui()
    ...
    def cambiar_periodo(self):
        ...
        #self.guardar_estado()
        if self.descendente:
            # Si es cuenta regresiva: volver al tiempo inicial configurado (o 30 min por defecto)
            self.tiempo = 1800
        else:
            # Si es cuenta ascendente (fútbol normal): volver a 0
            self.tiempo = 0
        self.lbl_tiempo.setText(self._format_time(self.tiempo))
        # --- En modo fútbol, al cambiar período: borrar minutos añadidos ---
        if self.deporte == "futbol":
            self.tiempo_añadido = 0
            if hasattr(self, "lbl_añadido"):
                self.lbl_añadido.hide()
    ...
    # Resets
    # ----------------------------
    def aplicar_reset(self):
        if self.opcion_reset_idx == 0:   # TODO
            self.reset_penales()
            self.reset_total()
        elif self.opcion_reset_idx == 1: # TIEMPO
            self.reset_tiempo()
        elif self.opcion_reset_idx == 2: # PENALES
            self.reset_penales()

    def reset_tiempo(self):
        # Reinicia solo el cronómetro y el añadido, manteniendo goles y faltas
        self.pausar_timer()
        
        if self.descendente:
            self.tiempo = self.tiempo_inicial if self.tiempo_inicial > 0 else 1800
        else:
            self.tiempo = 0
            
        # Si es fútbol, limpiar añadido también
        if self.deporte == "futbol":
            self.tiempo_añadido = 0
            if hasattr(self, "lbl_añadido"):
                self.lbl_añadido.hide()
    ...
    def reset_total(self):
        # full reset: goles, tiempo, periodo, penales, tarjetas
        self.goles_local = 0
        self.goles_visitante = 0
    ...
    def actualizar_ui_completa(self, startup=False):
        # 1. Actualizar textos de datos (siempre)
        self.lbl_local_score.setText(str(self.goles_local))
        self.lbl_visit_score.setText(str(self.goles_visitante))
        self.lbl_periodo.setText(self.periodos[self.periodo])
        self.lbl_modo.setText("Modo: DESCENDENTE" if self.descendente else "Modo: ASCENDENTE")
        ...
        # ----------------------------------------------------
        # SECTOR SUPERIOR: ¿Marcador Normal o Panel Global?
        # ----------------------------------------------------
        if self.modo_global:
            # Mostrar Global
            self.frame_global.setVisible(True)
        ...
        # ----------------------------------------------------
        # SECTOR MEDIO/INFERIOR: ¿Información de Tiempo o Penales?
        # ----------------------------------------------------
        if getattr(self, "penales_activo", False):
            # Mostrar Penales
            self.penales_frame.setVisible(True)
            
            # Ocultar Info de Tiempo
            self.lbl_tiempo.setVisible(False)
            self.lbl_estado.setVisible(False)
            self.lbl_periodo.setVisible(False)
            self.lbl_modo.setVisible(False)
        else:
            # Mostrar Info de Tiempo
            self.penales_frame.setVisible(False)
            ...
            # El modo solo se ve si no es fútbol
            if self.deporte == "futbol":
                self.lbl_modo.setVisible(False)
            else:
                self.lbl_modo.setVisible(True)
        ...
        # Refrescar elementos auxiliares
        self.actualizar_penales_ui()
        self.render_menu()
    ...
    def actualizar_ui_completa(self, startup=False):
        ...
        # mode
        self.lbl_modo.setText("Modo: DESCENDENTE" if self.descendente else "Modo: ASCENDENTE")
        #Lógica de visibilidad Global vs Normal ---
        if self.modo_global:
            ...
            self.frame_global.setVisible(True)
            self.actualizar_global_ui()
            # En modo global también ocultamos el modo tiempo
            self.lbl_modo.setVisible(False)
            
        # tiempo label
        elif self.penales_activo:
            self.lbl_tiempo.setVisible(False)
            self.lbl_estado.setVisible(False)
            self.lbl_periodo.setVisible(False)
            self.lbl_modo.setVisible(False)
    ...
    def actualizar_global_ui(self):
        if not self.modo_global:
            return

        # 1. Nombres (Actualizar texto por si cambió)
        txt_L = self.lbl_local_txt.text()
        txt_V = self.lbl_visit_txt.text()
        for i in range(3):
            getattr(self, f"lbl_glob_name_L_{i}").setText(txt_L)
            getattr(self, f"lbl_glob_name_V_{i}").setText(txt_V)

        # 2. Calcular valores
        val_ida_L = self.goles_ida_local
        val_ida_V = self.goles_ida_visitante
        val_vuelta_L = self.goles_local
        val_vuelta_V = self.goles_visitante
        val_glob_L = val_ida_L + val_vuelta_L
        val_glob_V = val_ida_V + val_vuelta_V

        # 3. Asignar textos a scores
        self.lbls_global_scores_L[0].setText(str(val_ida_L))
        self.lbls_global_scores_V[0].setText(str(val_ida_V))
        self.lbls_global_scores_L[1].setText(str(val_vuelta_L))
        self.lbls_global_scores_V[1].setText(str(val_vuelta_V))
        self.lbls_global_scores_L[2].setText(str(val_glob_L))
        self.lbls_global_scores_V[2].setText(str(val_glob_V))

        # 4. Estilos (Colores)
        style_norm = "color: black; font-weight: bold;"
        style_edit = "color: yellow; font-weight: bold;"
        style_winner = "color: lime; font-weight: bold;"  # <--- Nuevo estilo verde

        # A) Scores (Edición de Ida)
        for lbl in self.lbls_global_scores_L + self.lbls_global_scores_V:
            lbl.setStyleSheet(style_norm)

        if self.editando_global_fase == 1:
            self.lbls_global_scores_L[0].setStyleSheet(style_edit)
        elif self.editando_global_fase == 2:
            self.lbls_global_scores_V[0].setStyleSheet(style_edit)

        # B) Nombres (Ganador Penales) - NUEVO BLOQUE
        # Primero resetear todos los nombres a negro
        for i in range(3):
            getattr(self, f"lbl_glob_name_L_{i}").setStyleSheet(style_norm)
            getattr(self, f"lbl_glob_name_V_{i}").setStyleSheet(style_norm)

        # Si hay ganador, pintar solo el renglón Global (índice 2)
        if self.ganador == "L":
            getattr(self, f"lbl_glob_name_L_2").setStyleSheet(style_winner)
        elif self.ganador == "V":
            getattr(self, f"lbl_glob_name_V_2").setStyleSheet(style_winner)


        # 5. Ajuste de fuentes
        h = self.height()
        s_small = max(12, int(h * 0.035)) 
        s_big = max(24, int(h * 0.065))   

        font_s = QFont("Arial", s_small, QFont.Bold)
        font_b = QFont("Arial", s_big, QFont.Bold)

        # Aplicar fuentes
        for i in [0, 1]:
            self.lbls_global_scores_L[i].setFont(font_s)
            self.lbls_global_scores_V[i].setFont(font_s)
            getattr(self, f"lbl_glob_name_L_{i}").setFont(font_s)
            getattr(self, f"lbl_glob_name_V_{i}").setFont(font_s)
            self.lbls_global_titulos[i].setFont(font_s)
            self.lbls_global_dashes[i].setFont(font_s)

        # Fila 2 (Global)
        self.lbls_global_scores_L[2].setFont(font_b)
        self.lbls_global_scores_V[2].setFont(font_b)
        getattr(self, f"lbl_glob_name_L_2").setFont(font_b)
        getattr(self, f"lbl_glob_name_V_2").setFont(font_b)
        self.lbls_global_titulos[2].setFont(font_b)
        self.lbls_global_dashes[2].setFont(font_b)        
    ...
    def eventFilter(self, obj, event):
        ...
        # Si los nombres LOCAL o VISITANTE se mueven o cambian de tamaño
        if event.type() == QEvent.Move or event.type() == QEvent.Resize:
            if obj == self.lbl_local_txt or obj == self.lbl_visit_txt:
                self._actualizando = True
                self.actualizar_tarjetas_ui()
                self._actualizando = False
                
        # Si la ventana principal cambia (tu código original)
        if event.type() == QEvent.Resize and obj == self:
            self.adjust_font_sizes()
            self.aplicar_fondo()
            # Forzamos actualización también aquí por seguridad
            self.actualizar_tarjetas_ui()
    ...
    def adjust_font_sizes(self):
        ...
        if hasattr(self, "menu_frame") and self.menu_frame is not None:
            # Detectar si estamos en el modo "apretado"
            is_crowded = self.modo_global and getattr(self, "penales_activo", False)
            
            # Factor de altura del menú (30% normal, 20% si está apretado)
            factor = 0.20 if is_crowded else 0.30
            
            # Calcular altura final del menú
            menu_h = int(h * factor)
            self.menu_frame.setFixedHeight(menu_h)

            # --- AJUSTE FINO DEL CONTENIDO DEL MENÚ ---
            layout = self.menu_frame.layout()
            
            if is_crowded:
                # Modo Compacto: Letra más chica, márgenes mínimos
                menu_font_size = int(base * 0.65)  # 65% del tamaño normal
                layout.setSpacing(2)
                layout.setContentsMargins(4, 4, 4, 4)
            else:
                # Modo Normal: Letra base, márgenes cómodos
                menu_font_size = base
                layout.setSpacing(6)
                layout.setContentsMargins(8, 8, 8, 8)

            menu_font = QFont("Arial", max(8, menu_font_size))
            
            # Calcular altura disponible para cada renglón (son 3 visibles)
            # Altura total - márgenes - espacios / 3
            margins = layout.contentsMargins().top() + layout.contentsMargins().bottom()
            spacing = layout.spacing() * 2 # Hay 2 espacios entre 3 items
            available_h = menu_h - margins - spacing
            row_h = max(15, int(available_h / 3))

            for lbl in self.menu_labels:
                lbl.setFont(menu_font)
                # Forzar altura exacta para que entren los 3
